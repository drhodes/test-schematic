...
var cktsim = (function() {
        ...
        if (!this.finalized) {
            ...
            if (n_vsrc > 0) {
                ...
                if (rGV < n_vsrc) {
                    ...
                    alert(
                        'Warning!!! Simulator might produce meaningless results or no result with illegal circuits.'
                    );
                    return false;
                }
            }
    ...
    function mat_scale_add(A, B, scalea, scaleb, C /*out*/) {
        ...
        if (typeof scalea == 'number' && typeof scaleb == 'number')
            for (var i = 0; i < n; i++)
                for (var j = 0; j < m; j++) C[i][j] = scalea * A[i][j] + scaleb * B[i][j];
        else if (typeof scaleb == 'number' && scalea instanceof Array)
            for (var i = 0; i < n; i++)
                for (var j = 0; j < m; j++) C[i][j] = scalea[i] * A[i][j] + scaleb * B[i][j];
        else if (typeof scaleb instanceof Array && scalea instanceof Array)
            for (var i = 0; i < n; i++)
                for (var j = 0; j < m; j++) C[i][j] = scalea[i] * A[i][j] + scaleb[i] * B[i][j];
    ...
    // source types: dc,step,square,triangle,sin,pulse,pwl(piecewise linear),pwl_repeating
    ...
    function parse_source(v) {
        ...
        // square(v_init,v_plateau,freq,duty_cycle)
            ...
            pwl_source(
                src,
                [0, v1, t_change, v2, t_change + t_pw, v2, t_change + t_pw + t_change, v1, per, v1],
                true
            );
        }
    ...
    // TODO(???) small signal model: open circuit
...
schematic = (function() {
    ...
    function Schematic(input) {
        ...
        if (this.edits_allowed) {
            ...
            this.tools['copy'] = this.add_tool(
                copy_icon,
                'Copy: copy selected components into the clipboard',
                this.copy
            );
            this.tools['paste'] = this.add_tool(
                paste_icon,
    ...
    Schematic.prototype.zoomall = function() {
        ...
        if (sch_w == 0 && sch_h == 0) {
            ...
            this.scale = Math.pow(
                zoom_factor,
                Math.ceil(Math.log(Math.min(scale_x, scale_y)) / Math.log(zoom_factor))
            );
    ...
    Schematic.prototype.transient_analysis = function() {
        ...
        this.dialog('Transient Analysis', content, function(content) {
            ...
            var results = ckt.tran(
                ckt.parse_number(sch.tran_npts),
                0,
                ckt.parse_number(sch.tran_tstop),
                probe_names,
                false
            );
    ...
    Part.prototype.draw_arc = function(
        c,
        x,
        y,
        radius,
    ...
    function VCCSource(x, y, rotation, name, conductance) {
        ...
        this.properties['name'] = name;
        if (conductance == undefined) conductance = '1';
    ...
    VCCSource.prototype.draw = function(c) {
        ...
        if (this.properties['conductance']) {
            const label = 'G(' + this.properties['conductance'] + ')';
            this.draw_text(c, label, 13, 24, 3, property_size);
        }
    };
    ...
    VCCSource.prototype.build_content = function(src) {
        ...
        var fields = [];
        fields['name'] = build_input('text', 10, this.properties['name']);
        fields['G'] = build_input('conductance', 10, this.properties['conductance']);
        var div = this.content;
        if (div.hasChildNodes()) div.removeChild(div.firstChild); // remove table of input fields
        div.appendChild(build_table(fields));
        div.fields = fields;
    ...
    VCCSource.prototype.edit_properties = function(x, y) {
        if (this.near(x, y)) {
            ...
            this.sch.dialog('Edit Properties', content, function(content) {
                ...
                c.properties['conductance'] = fields['G'].value;
                c.sch.redraw_background();
            });
            return true;
        } else return false;
